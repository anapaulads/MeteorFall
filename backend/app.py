...
def calculate_asteroid_mass(avg_diameter_m: float, density_kg_m3: float = 2000) -> float:
    if not avg_diameter_m or avg_diameter_m == 0:
        return 0.0
...
def calculate_kinetic_energy(mass_kg: float, velocity_km_s: float) -> dict:
    if not mass_kg or not velocity_km_s:
        return {"joules": 0.0, "megatons_tnt": 0.0}
...
def get_weekly_asteroids():
    ...
    try:
        ...
        for neo in all_neos:
            ...
            size_m = safe_float(neo.get("estimated_diameter", {}).get(
                "meters", {}).get("estimated_diameter_max", 0))
            velocity_km_s = safe_float(approach_data.get(
                "relative_velocity", {}).get("kilometers_per_second", 0))
